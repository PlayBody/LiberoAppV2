...
class _ConnectReserveConfirm extends State<ConnectReserveConfirm> {
  ...
  DateTime reserveFromTime = new DateTime.now();
  ...
  Future<List> loadReserveData() async {
    ...
    if (widget.isNoReserveType == constCheckinReserveRiRa) {
      reserveFromTime = widget.reserveStartTime;
  ...
  Future<void> saveReserveData() async {
    ...
    globals.connectReserveMenuList.forEach((e) {
      menuIds.add(e.menuId);
    });
    param['menu_ids'] = menuIds.join(',');
    ...
    param['use_ticket'] = jsonEncode(ticketdata).toString();
    print(param);
