...
class _ReserveDateSecond extends State<ReserveDateSecond> {
  ...
  Future<List> loadInitData() async {
    ...
    // regions = await ClReserve().loadReserveTimeStatus(context, widget.organId,
    //     widget.staffId, _fromDate, _toDate, widget.isNoReserveType, '5');
    ...
    regions = await ClReserve().loadReserveTimeStatus(
        ...
        '',
        widget.staffId ?? '',
        DateFormat('yyyy-MM-dd').format(widget.selTime),
        DateFormat('yyyy-MM-dd').format(widget.selTime),
        true);
